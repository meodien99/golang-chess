...
func TestEval(t *testing.T){
	...
	board.SetUpPieces()
	if eval := EvalBoard(board); eval >= 0.1 ||eval <= -0.1 {
		// the exact value is expected to be -0.0xxxx ... which isn't 0
		t.Errorf("Initial position has evaluation of %f, expecting %f", eval, DRAW)
	}
}


func TestKingIsSafety(t *testing.T){
	if score := checkKingSafety(1, []chess.Square{}); score > 0 {
		t.Errorf("Isolated king in corner, gives positive score of %f", score)
	}
}

func TestPawnIsPassed(t *testing.T){
	oppFullPawns := []chess.Square{}
	oppFullPawns = append(oppFullPawns, chess.Square{2, 5})
	pawn := &chess.Piece{
